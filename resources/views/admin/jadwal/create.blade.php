                ...
                <form action="{{ route('admin.jadwal.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="ruang" id="ruang" value="{{ old('ruang') }}"
                                   list="ruang_list"
                                   class="w-full rounded-lg border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500"
                    ...
                    <div class="mb-4">
                        ...
                        <div class="relative">
                            <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', date('Y-m-d')) }}"
                                   class="w-full rounded-lg border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500"
                                   required>
                            <div id="hari-output" class="mt-2 text-sm font-bold text-red-600 flex items-center gap-1">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                </svg>
                                <span id="nama-hari">Menghitung hari...</span>
                            </div>
                        </div>
                        @error('tanggal')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        <div>
                            ...
                            <input type="time" name="jam_mulai" id="jam_mulai" value="{{ old('jam_mulai', '08:30') }}"
                                   class="w-full rounded-lg border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500"
                                   required>
                            @error('jam_mulai')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <input type="time" name="jam_selesai" id="jam_selesai" value="{{ old('jam_selesai', '10:30') }}"
                                   class="w-full rounded-lg border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500"
                                   required>
                            @error('jam_selesai')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateNamaHari() {
            ...
            namaHariSpan.innerText = "Hari Terpilih: " + dayName;
        }

        // Jalankan saat load & saat input berubah
        updateNamaHari();
